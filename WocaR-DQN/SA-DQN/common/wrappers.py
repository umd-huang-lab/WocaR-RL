...
def atari_env(env_id):
    ...
    if 'FIRE' in env.unwrapped.get_action_meanings():
        env = FireResetEnv(env)
    env = AtariRescale(env, crop=20)
    return env


def process_frame(frame, crop):
...
class AtariRescale(gym.ObservationWrapper):
    def __init__(self, env, crop=20):
        gym.ObservationWrapper.__init__(self, env)
        self.observation_space = spaces.Box(0.0, 1.0, [1, 80, 80], dtype=np.uint8)
        self.crop = crop

    def observation(self, observation):
        return process_frame(observation, self.crop)