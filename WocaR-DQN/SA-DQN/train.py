...
from ibp import worst_action_select
...
def compute_td_loss(current_model, target_model, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, **kwargs):
    ...
    '''
    '''
    worst_eps = kwargs['worst_eps']
    worst_kappa = kwargs['worst_kappa']
    with torch.no_grad():
        cur_q_logits_ub, cur_q_logits_lb = current_model.bound_forward(state, eps=worst_eps)

    action_labels = torch.zeros(cur_q_logits_ub.shape).cuda()
    action_labels[range(state.shape[0]), action] = 1
    cur_ub_diff = cur_q_logits_ub - cur_q_logits * (1-action_labels) - expected_q_value.detach().unsqueeze(1)*action_labels
    cur_lb_diff = cur_q_logits_lb - cur_q_logits * (1-action_labels) - expected_q_value.detach().unsqueeze(1)*action_labels
    worst_case = torch.max(torch.abs(cur_ub_diff), torch.abs(cur_lb_diff))
    worst_case_loss = (torch.sum(torch.min(worst_case.pow(2), worst_case), dim=1).mean())/10
    '''
    ...
    loss = loss.mean()
    # loss = (worst_kappa * loss +(1-worst_kappa) * (worst_case_loss))
    td_loss = loss.clone()
...
def compute_td_loss_worst(current_model, target_model, worst_model, worst_optimizer, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, **kwargs):
    t = time.time()
    dtype = kwargs['dtype']
    if per:
        buffer_beta = kwargs['buffer_beta']
    ...
    with torch.no_grad():
        tgt_worst_q_logits = worst_model(next_state)
        # tgt_worst_q_value = tgt_worst_q_logits.max(1)[0]
    count = kwargs['training_count']
    worst_rate = kwargs['worst_rate']
    worst_ratio = kwargs['worst_ratio']
    worst_eps = kwargs['worst_eps']
    worst_kappa = kwargs['worst_kappa']

    tgt_next_q_value = tgt_next_q_logits.max(1)[0]
    
    weighted_next_q_logits = (1 - worst_ratio) * tgt_next_q_logits + worst_ratio * tgt_worst_q_logits
    _ , next_max_action = torch.max(weighted_next_q_logits, dim=1)

    tgt_next_q_value = tgt_next_q_logits.gather(1, next_max_action.unsqueeze(1)).squeeze(1)
    
    expected_q_value = reward + gamma * tgt_next_q_value * (1 - done)

    # worst q training
    if count % worst_rate == 0:
        worst_model_training = True
    else:
        worst_model_training = False

    cur_q_logits_ub, cur_q_logits_lb = current_model.bound_forward(state, eps=worst_eps)

    if worst_model_training:
        worst_q_logits = worst_model(state)
        worst_q_value = worst_q_logits.gather(1, action.unsqueeze(1)).squeeze(1)
        next_q_logits_ub, next_q_logits_lb = current_model.bound_forward(next_state, eps=1e-7)

        worst_next_q_value = worst_action_select(tgt_worst_q_logits, next_q_logits_ub, next_q_logits_lb)
        exp_worst_q_value = reward + gamma * worst_next_q_value * (1 - done)
    ...
    cur_lb_diff = cur_q_logits_lb - cur_q_logits * (1-action_labels) - expected_q_value.detach().unsqueeze(1)*action_labels
    worst_case = torch.max(torch.abs(cur_ub_diff), torch.abs(cur_lb_diff))
    worst_case_loss = torch.sum(torch.min(worst_case.pow(2), worst_case), dim=1)
    worst_case_loss = (worst_case_loss * weights).mean()
    # worst_case_loss_fn = torch.nn.SmoothL1Loss(reduction='none')
    # worst_case_loss = worst_case_loss_fn(worst_case_loss, torch.zeros_like(worst_case_loss)).mean()
    
    if kwargs['natural_loss_fn'] == 'huber':
        loss_fn = torch.nn.SmoothL1Loss(reduction='none')
        loss = loss_fn(cur_q_value, expected_q_value.detach())
        if worst_model_training:
            worst_loss_fn = torch.nn.SmoothL1Loss(reduction='none')
            worst_loss = worst_loss_fn(worst_q_value, exp_worst_q_value.detach())
    else:
        loss  = (cur_q_value - expected_q_value.detach()).pow(2)
        if worst_model_training:
            worst_loss  = (worst_q_value - exp_worst_q_value.detach()).pow(2)
    if per:
        loss = loss * weights
        prios = loss + 1e-5
        weights_norm = np.linalg.norm(numpy_weights)
    ...
    if worst_model_training:
        batch_worst_q_value = torch.mean(worst_q_value)
        worst_loss = worst_loss.mean()
        td_worst_loss = worst_loss.clone()
    ...
    grad_norm = 0.0
    max_norm = kwargs['grad_clip']
    if max_norm > 0 and worst_model_training:
        parameters = worst_model.parameters()
        for p in parameters:
            grad_norm += p.grad.data.norm(2).item() ** 2
        grad_norm = np.sqrt(grad_norm)
        clip_coef = max_norm / (grad_norm + 1e-6)
    ...
    res = (loss, grad_norm, weights_norm, td_loss, batch_cur_q_value, batch_exp_q_value, worst_case_loss)
    if worst_model_training: 
        res += (worst_loss, td_worst_loss, batch_worst_q_value)
    if robust:
        if bound_solver == 'pgd':
            res += (ori_margin, adv_margin)
        res += (reg_loss,)
...
def main(args):
    ...
    if "NoFrameskip" not in env_id:
        ...
        env = make_atari(env_id)
        env = wrap_deepmind(env, **env_params)
        #env = AtariRescale(env, **env_params)
        env = wrap_pytorch(env)
    ...
    worst_training = training_config.get('worst', False)
    if worst_training:
        print("Worst value counts!!!")
        worst_rate = training_config['worst_rate']
        worst_ratio = training_config['worst_ratio']
        worst_eps_end = training_config['worst_eps_end']
        worst_schedule = training_config['worst_schedule']
        worst_kappa_end = training_config['worst_kappa_end']
        worst_model = model_setup(env_id, env, robust_model, logger, USE_CUDA, dueling, model_width)
        worst_eps = np.nan
        worst_kappa = np.nan
    ...
    if worst_training:
        worst_optimizer = optim.Adam(worst_model.parameters(), lr=lr, eps=training_config['adam_eps'])
    ...
    for frame_idx in range(load_frame, num_frames + 1):
        ...
        coeff = min(1, (frame_idx+1)/max((num_frames + 1-load_frame), worst_schedule))
        worst_eps = coeff * worst_eps_end
        worst_kappa = (1 - coeff)*1 + coeff*worst_kappa_end
        if buffer_stored_size > replay_initial:
            ...
            if worst_training:
                training_count += 1
                res = compute_td_loss_worst(current_model, target_model, worst_model, worst_optimizer, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, worst_eps = worst_eps, worst_ratio = worst_ratio, worst_rate = worst_rate, worst_kappa = worst_kappa, training_count = training_count, buffer_beta=buffer_beta, grad_clip=grad_clip, natural_loss_fn=natural_loss_fn, eps=eps, beta=beta, sa=sa, kappa=kappa, dtype=dtype, hinge=hinge, hinge_c=training_config.get('hinge_c', 1), env_id=env_id, bound_solver=bound_solver, attack_config=attack_config)
                worst_case_loss, worst_loss, td_worst_loss, batch_worst_q_value = res[6], res[7], res[8], res[9]
                if training_count % worst_rate == 0:
                    worst_losses.append(worst_loss.data.item())
                    td_worst_losses.append(td_worst_loss.data.item())
                    worst_case_losses.append(worst_case_loss.data.item())
                    batch_worst_q.append(batch_worst_q_value.data.item())
            else:
                res = compute_td_loss(current_model, target_model, batch_size, replay_buffer, per, use_cpp_buffer, use_async_rb, optimizer, gamma, memory_mgr, robust, buffer_beta=buffer_beta, grad_clip=grad_clip, natural_loss_fn=natural_loss_fn, eps=eps, beta=beta, sa=sa, kappa=kappa, dtype=dtype, hinge=hinge, hinge_c=training_config.get('hinge_c', 1), env_id=env_id, bound_solver=bound_solver, attack_config=attack_config, worst_eps=worst_eps, worst_kappa=worst_kappa)
        ...
        if frame_idx % training_config['print_frame'] == 0 or frame_idx==num_frames or (robust and abs(frame_idx-schedule_start) < 5) or abs(buffer_stored_size-replay_initial) < 5:
            print("normal train", len(losses))
            if worst_training:
                print("worst_train", len(worst_losses))
            logger.log('\nframe {}/{}, learning rate: {:.6g}, buffer beta: {:.6g}, action epsilon: {:.6g}'.format(frame_idx, num_frames, lr, buffer_beta, act_epsilon))
            if time.time() - period_start_time == 0:
                time_gap = 0.01
            else:
                time_gap = time.time() - period_start_time
            logger.log('total time: {:.2f}, epoch time: {:.4f}, speed: {:.2f} frames/sec, last total loss: {:.6g}, avg total loss: {:.6g}, grad norm: {:.6g}, weights_norm: {:.6g}, latest episode reward: {:.6g}, avg 10 episode reward: {:.6g}'.format(
                ...
                training_config['print_frame'] / time_gap,
            ...
            if worst_training:
                logger.log('last td worst loss: {:.6g}, avg td worst loss: {:.6g}'.format(
                        td_worst_losses[-1] if td_worst_losses else np.nan,
                        np.average(td_worst_losses[:-training_config['print_frame']-1:-1]) if td_worst_losses else np.nan))
                logger.log('last batch worst q: {:.6g}, avg batch worst q: {:.6g}'.format(
                        batch_worst_q[-1] if batch_worst_q else np.nan,
                        np.average(batch_worst_q[:-training_config['print_frame']-1:-1]) if batch_worst_q else np.nan))
                logger.log('current worst epsilon: {:.6g}, current worst kappa: {:.6g}'.format(worst_eps, worst_kappa))
                logger.log('last td worst-case loss: {:.6g}, avg td worst-case loss: {:.6g}'.format(
                        worst_case_losses[-1] if worst_case_losses else np.nan,
                        np.average(worst_case_losses[:-training_config['print_frame']-1:-1]) if worst_case_losses else np.nan))
