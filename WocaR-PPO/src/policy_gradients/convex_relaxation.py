...
if forward_one:
    def get_kl_bound(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
        ...
        if use_full_backward:
            ...
            # IBP Pass.
            ilb, iub = model.compute_bounds(IBP=True)
    ...
    def get_kl_bound(model, x, means, eps):
        ...
        if forward_one:
            ...
            inputs = (x, means)
        # IBP Pass.
        _, iub = model.compute_bounds(inputs, IBP=True, method=None, bound_lower=False, bound_upper=True)
        # CROWN Pass.
        _, cub = model.compute_bounds(x=None, IBP=False, method='backward', bound_lower=False, bound_upper=True)
        # iub = cub
        return iub, cub
